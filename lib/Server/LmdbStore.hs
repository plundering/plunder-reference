...
data LmdbStore = LmdbStore
    ...
    , lmdbStoreMachineEventlogTables :: !(Map MachineName MachineLogTable)
...
instance Show MachineLogTable where
    show MachineLogTable{..} = unsafePerformIO do
        n <- readTVarIO numBatches
...
close _dir LmdbStore{..} = do
  mdb_dbi_close lmdbStoreEnv lmdbStoreMachineTable
  mdb_dbi_close lmdbStoreEnv lmdbStoreBackupsTable
  for_ (mapToList lmdbStoreMachineEventlogTables) \(_,s) ->
    mdb_dbi_close lmdbStoreEnv (logTable s)
...
getAllMachines txn machineTable =
    with (cursor txn machineTable) \cur ->
        forRange cur RSBegin \pKey _ -> do
            ...
            pure $ MachineName $ decodeUtf8 key
...
getAllMachinesInStore :: StateT LmdbStore IO [MachineName]
...
writeLogBatch who lb@LogBatch{..} = do
  ...
  case lookup who logTables of
    ...
    Just MachineLogTable{..} -> do
      ...
      let next = batch + 1
      writeIt batch logTable lbHash lbNoun pinz
  ...
  where
    ...
    writeIt curBatch logTable _lbHash lbNoun pinz = do
      ...
      liftIO $ with (writeTxn env) $ \txn -> do
        for_ pinz (stickPinInCushion txn pinCushion)
...
loadPin :: MDB_txn -> MDB_dbi -> ByteString -> IO P.Fan
...
data LoggingEvent
    = LogGetAllMachines ([MachineName] -> STM ())
...
lmdbRequest LmdbThread{..} op = do
    v <- atomically do
        v <- newEmptyTMVar
        writeTQueue' lmdbThreadEvent $ op (putTMVar v)
        pure v
    atomically (takeTMVar v)

getMachineNames :: LmdbThread -> IO [MachineName]
...
queueReplayFrom
    :: LmdbThread -> MachineName -> ReplayFrom -> TBMQueue LogBatch
    -> STM ()
queueReplayFrom LmdbThread{..} tn rf q =
    writeTQueue' lmdbThreadEvent (LogReplayFrom tn rf q)

queueWriteLogBatch :: LmdbThread -> MachineName -> LogBatch -> STM () -> STM ()
